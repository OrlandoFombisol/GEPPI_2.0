...
import { evidenciaItemSgsstDB } from '@/db'
...
/**
 * Panel de evidencias adjuntas a un ítem del checklist SG-SST 0312.
 * Cada ítem (codigo) de cada evaluación (empresa + año) tiene su propia
 * carpeta de evidencias en Storage.
 */
export default function EvidenciaItemPanel({ evaluacionId, codigo, empresaId, itemLabel, usuarioId, onClose }) {
  ...
  useEffect(() => { cargar() }, [evaluacionId, codigo])

  async function cargar() {
    setLoading(true)
    try {
      const data = await evidenciaItemSgsstDB.getByItem(evaluacionId, codigo)
      setEvidencias(data || [])
  ...
  async function handleFile(e) {
    ...
    setSubiendo(true)
    try {
      await evidenciaItemSgsstDB.upload(evaluacionId, codigo, empresaId, file, usuarioId)
      await cargar()
    } catch (err) {
  ...
  async function handleVer(ev) {
    const url = await evidenciaItemSgsstDB.getUrl(ev.storagePath)
    window.open(url, '_blank')
  }

  async function handleEliminar(ev) {
    await evidenciaItemSgsstDB.remove(ev.id, ev.storagePath)
        ...
        <div className="flex items-start justify-between gap-3 p-5 border-b border-slate-100">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              <p className="text-sm font-bold text-slate-800">Evidencias — {codigo}</p>
              <p className="text-xs text-slate-500 mt-0.5 line-clamp-2">{itemLabel}</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-2">
          {loading ? (
            ...
            <div className="flex flex-col items-center gap-2 py-10 text-center">
              ...
              <p className="text-xs text-slate-300">Sube el primer archivo de soporte para este ítem</p>
            </div>
          ) : (
            evidencias.map(ev => (
        ...
        <div className="p-5 border-t border-slate-100 space-y-3">
          ...
          {lleno ? (
            <div className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 text-center">
              Límite alcanzado — máximo 5 evidencias por ítem
            </div>
          ) : (
            <button
          ...
          <p className="text-center text-xs text-slate-400">
            Máx. 5 MB por archivo · JPG, PNG, PDF, Excel · Máx. 5 archivos por ítem
          </p>
        </div>
      </div>
    </div>
